...
ASSUMPTIONS: tuple[Assumption, ...] = (
    ...
    Assumption(
        ...
        statement="pugliese_rate_malecns_v1 uses the Pugliese half-tanh rate ODE, truncated-normal (tau, gain, threshold, rmax) draws with split(PRNGKey(seed), 5) when JAX is available, excitatory/inhibitory multipliers 0.03, and W_rate = signed_W.T. MaleCNS size is neuPrint Neuron.size (voxel volume), normalized by the median of all male-cns:v1.0 neurons with a positive size, not by the 408-cell circuit median and not by SWC/synapse-count/cable/soma/partners. Stimulus amplitude 250 is a transfer assumption: no mCNS stimI was recovered (MANC yaml is 250, FANC is 150). The graph keeps both DNg100 cells; only MaleCNS 10056 (MANC homolog 10093 / matrix index 31) is driven. 10045 is not pooled. A rhythm on this restricted graph is a transfer test, not an intact-CNS result, and does not unlock full MaleCNS or body control.",
