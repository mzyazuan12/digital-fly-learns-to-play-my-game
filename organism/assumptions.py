...
ASSUMPTIONS: tuple[Assumption, ...] = (
    ...
    Assumption(
        ...
        statement="MaleCNS contains DNg100 (two neurons, one per side, resolved from type==DNg100), DNb08, DNg97/oDN1, and the walking CPG types from the Pugliese et al. 2025 bioRxiv preprint (E1=IN17A001, E2=INXXX466, I1=IN16B036, I2=IN19B007, E3=IN19B012, E4=IN03A006, E5=INXXX464). IN19A007 is not I2. Pugliese DNg100_Stim injects MANC T1 matrix index 31 / MANC body 10093 (type DNg100). In that MANC table body 10056 is vMS16. MaleCNS DNg100 is a separate lookup; the same integer is not the same cell. These names tag cells for measurement/lesion; they never mean if DNg100: walk(). Type-level averages are not a CPG state — CPG types are assigned from each bodyId's T1/T2/T3 × L/R ROI counts (PreSyn+PostSyn), not soma XYZ. MODE_NEURAL_CPG records E1/E2/I1 timing and does not actuate FlyBody yet. Standing is silent locomotor DNs, not a stand() command. An older preprint passage appears to call E5 INXXX466; canonical mapping is INXXX464.",
